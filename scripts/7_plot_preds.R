...
alb_diff = data.frame(matrix(NA, nrow=0, ncol=9))
colnames(alb_diff) = c('cell_id', 'long', 'lat', 'x', 'y', 'year', 'alb_pred', 'alb_bin', 'alb_diff')
...
for (i in 1:N_cells){
  ...
  alb_diff = rbind(alb_diff, data.frame(alb_cell_filled[1:(nrow(alb_cell_filled)-1), ], 
                                alb_diff = -diff(alb_cell_filled$alb_mean)))
  
}

alb_diff =  alb_diff[which(!is.na(alb_diff$diff)),]

saveRDS(alb_diff, paste0('data/preds_alb_diffs_sub_', alb_prod, '.RDS'))
