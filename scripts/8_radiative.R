...
# alb_diff$facets = labels[match(alb_diff$year, diff_years)]
alb_diff$facets = factor(alb_diff$facets, labels =  labels_period)
...
boxes_ENA = data.frame(yhi = 67, ylo = 48, xlo = -100, xhi = -52, id="ENA")
...
boxes_HEM = data.frame(yhi = 48, ylo = 37, xlo = -90, xhi = -60, id="HEM")
...
boxes_WCAN = data.frame(yhi = 75, ylo = 45, xlo = -165, xhi = -105, id="WCAN")
...
ggplot() +
  ...
  facet_grid(type~., scales="free_y", space = "free")
ggsave(paste0('figures/alb_preds_radiative_bar_regions_facet_', alb_prod, '.png'))
ggsave(paste0('figures/alb_preds_radiative_bar_regions_facet_', alb_prod, '.pdf'))
